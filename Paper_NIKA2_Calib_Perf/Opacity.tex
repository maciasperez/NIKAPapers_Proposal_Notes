...
For each {\tt skydip} scan and %for each bunch of 40 KIDs,
for each KID, 
...
($c_0^k$, $c_1^k$) parameters. Then, we determine two indicators of the fit
quality per {\tt skydip}. First, for each bunch of 40 KIDs, the
standard deviation of the measure-to-model difference $\sigma_{40}$ is calculated
over all the KIDs of the bunch. For each {\tt skydip}, we evaluate the
...
then converted from Hertz to Kelvin using the corresponding $c_1^k$
parameter. This cross-calibration allows for assembling the
...
under investigation. {\lp Beside the $c_1^k$-to-$\taunu$ degeneracy and the effect
