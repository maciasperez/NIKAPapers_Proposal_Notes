...
%MWC349A) %the two largest asteroids Ceres and Vesta were also observed.
...
determined. Then, we evaluate these calibrator measured-to-expected
flux density ratio using the baseline calibration method, and compare
to results drawn from other calibration methods in
Sect.~\ref{se:valid_baseline_cal}. The calibration statistical errors
are discussed in Sect.~\ref{se:valid_photocorr_cal}. 
Finally, in Sect.~\ref{se:cal_JFL}, we check the robustness of the calibration
results using an alternative calibration method that relies on
aperture photometry.


\section{Reference flux densities of the secondary calibrators {\color{blue} Jean-Fran\c cois}}
