...
%========== Pressure from tSZ
\subsection{Thermal Sunyaev-Zel'dovich pressure mapping}\label{sec:Thermal_Sunyaev_Zeldovich_pressure_mapping}
...
\label{eq:pseudo_p}
\end{equation}
This quantity is straightforwardly obtained from the NIKA map accounting for relativistic corrections as detailed in \cite{Adam2016b}. As the temperature can be very high, the relativistic corrections are non negligible \citep{Pointecouteau1998,Itoh2003}, but the exact choice of the temperature map used to apply relativistic corrections has a negligible impact on our results {\bf (i.e., the spectroscopic temperature maps from XMM-\textit{Newton}, from \textit{Chandra},  or $\bar{T}_{\rm gmw}$)}. 

{\bf The zero level of the NIKA map is defined by using the Planck catalog \citep[PSZ2,][]{Planck2015XXVII} normalization. The flux -- angular size constraint, in the plane $Y_{5 R_{500}}$ -- $\theta_s$, is marginalized over $\theta_s$ to obtain the flux within $5 R_{500}$, $Y_{5 R_{500}} = 3.04_{-0.49}^{+0.53} \times 10^{-3}$ arcmin$^2$. Following \citep{Adam2015}, we constrain the zero level of the map by fitting a generalized Navarro Frenk White pressure profile \citep{Nagai2007b} to the NIKA data, which we normalize by the Planck flux. We use various slope parameter models to extrapolate the signal on large scales \citep[from][but also by fitting for the corresponding parameters]{Nagai2007b,Arnaud2010,Planck2013V} and test several configurations (free or fixed cluster center, correcting or not for kSZ). We obtain an average zero level of 0.80 mJy/beam and estimate its systematic uncertainty to be 0.10 mJy/beam from the different test models we use.}

The right panel of Figure \ref{fig:Input_maps} presents the $\bar{P}_e$ map in the case of model M1, {\bf thus $\propto \int P_e dl$, corrected} for kSZ and zero level. {\bf The pressure distribution also indicates that the cluster is disturbed. The morphology is similar to that of the density on large scales, but we observe strong differences at the substructure level, which indicate spatial variations of the temperature. We note in particular that the pressure peak is offset $\sim 30$ arcsec southeast with respect to the density peak.}
...
\caption{\footnotesize{Correlation between the temperature maps of Figure \ref{fig:T_maps}. The black dashed line provides the one to one correlation. {\bf Left:} XMM-\textit{Newton} versus \textit{Chandra} spectroscopic temperatures. {\bf Middle:} tSZ+X-ray imaging (model M1) versus XMM-\textit{Newton} spectroscopy. {\bf Right:} tSZ+X-ray imaging (model M1) versus \textit{Chandra} spectroscopy. The red and green dots correspond to the case with and without the kSZ correction, respectively.}}
\label{fig:T_SZ_T_X_correlation}
\end{figure*}

Figure \ref{fig:T_SZ_T_X_correlation} shows the correlation between the maps shown in Figure \ref{fig:T_maps}, {\bf compared to the one-to-one correlation shown by the black dashed line}. The temperature values are extracted in each {\bf 20} arcsec pixel, where the resolution of the $\bar{T}_{\rm gmw}$ map is 22 arcsec. We mask pixels where the tSZ signal-to-noise ratio is lower than 2 to avoid possible bouncing effects on the edge of the map due to the NIKA data processing.

{\bf As expected, we observe a good correlation between XMM-\textit{Newton} and \textit{Chandra} spectroscopic temperatures, but \textit{Chandra} temperatures are higher by about 15\%. The correlation between $\bar{T}_{\rm gmw}$ and XMM-\textit{Newton} spectroscopy is excellent. \textit{Chandra} spectroscopic temperatures are on overall lower than $\bar{T}_{\rm gmw}$. We note that the tSZ+X-ray error bars are significantly smaller than the X-ray spectroscopic ones, especially in low density and high temperature regions, despite the tSZ integration time that is lower by a factor of about 3.}

The tSZ+X-ray imaging versus X-ray only {\bf correlation} is stable to within 10\%, depending on the choice of the $\ell_{\rm eff}$ model and the kSZ correction used. {\bf Changing the NIKA map zero level within its uncertainty also leads to a change of about 10\% in the absolute level of the temperature.} The {\bf correlation also directly depends} on the absolute calibration of the tSZ map, which is expected to be accurate within 7\%. Model M3b provides a lower limit on $\ell_{\rm eff}$, and therefore an upper limit on {\bf the $\bar{T}_{\rm gmw}$ map. The changes in the temperature comparison, as a function of the considered systematic effects, are illustrated in Appendix \ref{sec:appendix1} and \ref{sec:appendix2}.}
...
{\bf 
\appendix
\section{Impact of the $\ell_{\rm eff}$ model on the temperature comparison}\label{sec:appendix1}
Figure \ref{fig:show_leff_effect} illustrates the evolution of the temperature comparison between tSZ+X-ray and the X-ray spectroscopic ones, as a function of the various $\ell_{\rm eff}$ models considered in this paper, in the case of XMM-\textit{Newton} spectroscopic temperatures. Model M3b should be taken as a lower limit on $\ell_{\rm eff}$, and thus an upper limit on the tSZ+X-ray temperature.
\begin{figure}[h]
\includegraphics[width=0.24\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff1.pdf}
\includegraphics[width=0.24\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff5.pdf}
\includegraphics[width=0.24\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff8.pdf}
\includegraphics[width=0.24\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff9.pdf} 
\caption{\footnotesize{Impact of the gas density geometry model on the correlation of the temperature maps. The models, $\ell_{\rm eff}$, are M1 (top left), M2 (top right), M3a (bottom left) and M3b (bottom right).}}
\label{fig:show_leff_effect}
\end{figure}

\section{Impact of the NIKA map zero level on the temperature comparison}\label{sec:appendix2}
Figure \ref{fig:show_zl_effect} shows the impact of the NIKA map zero level on the correlation between the tSZ+X-ray temperatures and the X-ray spectroscopic ones. The NIKA map zero level is estimated to $0.8 \pm 0.1$ mJy/beam (see Section \ref{sec:Thermal_Sunyaev_Zeldovich_pressure_mapping}). Here, we provide the correlation for upper (1.0 mJy/beam) and lower (0.6 mJy/beam) limits of the zero level, compared to the baseline value of 0.8 mJy/beam. The changes in the zero level result in an offset in the tSZ+X-ray temperature map, with some dependence on the temperature attributed to the gas density spatial variations.
\begin{figure}[h]
\includegraphics[width=0.28\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff1_dd.pdf}
\includegraphics[width=0.28\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff1.pdf}
\includegraphics[width=0.28\textwidth]{Figure/Thermo_correlation_TXMM-TSZ_leff1_uu.pdf}
\caption{\footnotesize{Impact of the NIKA map zero level on the temperature comparison, shown in the case of tSZ+X-ray imaging (model M1) versus XMM-\textit{Newton} spectroscopy. From top to bottom, the zero level is 0.6, 0.8 and 1.0 mJy/beam, which widely samples the systematic uncertainty in the NIKA map zero level.}}
\label{fig:show_zl_effect}
\end{figure}
}
\end{document}