...
%\footnote{See https://ftp.sciops.esa.int/pub/hsc-calibration/PlanetaryModels/ESA4}
...
%These frequencies are close to the central frequencies of the NIKA2 channels, which are given in Table~\ref{sumperf}. 
...
%both by fitting a fixed FWHM (12.5$^{\prime \prime}$ and 18$^{\prime \prime}$ for the 260 and 150 GHz channels, respectively) 2D gaussian to the planet emission, and by aperture photometry within a radius of  $150^{\prime \prime}$ where cumulative flux density levels off smoothly. We find consistent results between the two estimates, with differences below 2 \%. In both cases the obtained fluxes were corrected for atmospheric absorption using the atmospheric line-of-sight opacity for the two NIKA2 channels, which was computed as described in \ref{Atmospheric attenuation correction}. \\
...
%for 2mm PWV and elevation $\delta = 60^{\circ}$
...
%     \tablefoottext{2}{Number of valid detectors times the area on the sky per detector.}
   ...
   \tablefoottext{3}{Achieved NEFD during the February 2017 observation campaign.}   
   \tablefoottext{4}{Achieved mapping speed during the February 2017 observation campaign.}   
  }
\end{table*}
