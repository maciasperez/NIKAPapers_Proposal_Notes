...
refractive portion of the optics, for a total mass around 100 kg,
...
Martin-Puplett interferometer built in-house
\citep{Durand2007_these}. Both
...
Hz. Ingested into the calibration pipeline, it will be futher converted into astronomical units
...
the atmospheric transmission or in the elevation) contributes as well to
...
observation in order to be optimally set at the same elevation and sky
conditions as the source. This process takes only a few seconds
whenever the resonances are close to the current functionning
point. These optimal conditions are further maintained by performing
continuous tunings between two scans while NIKA2 is not observing. 
