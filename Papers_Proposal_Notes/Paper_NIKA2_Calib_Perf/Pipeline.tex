...
installed for the data analysis during the commissioning campaigns and
is currently used for science-purpose data reduction. It implements
...
utilisation and flagged out potentially misbehaving KIDs or
timeline accidents (glitches). Specifically, we flag out cosmic rays,
which impact only one sample due to the KID fast time constant, periods
dedicated to the KID resonance frequency tuning (Sect.~\ref{se:tuning}), %of the KID tones to their resonance frequency
and the timeline of KIDs whose noise level exceed $3\sigma$ of the average
...
respect to the target source (usually located at the center of the
...
which is a key information for the treatment of correlated noises.\\
...
\noindent \emph{Correlated noise subtraction:}
\label{se:toi_proc}
The TOI of each KID include a large low-frequency component of correlated noise of two
different origins: the atmospheric component, which is usually
dominant and common to all KIDs, and the electronic noise, which is common to the KIDs connected
...
dedicated methods for this endavour. This will be thoroughtly
discussed in \citet{Ponthieu2019}, whereas here we illustrate the
general principle and only discussed the method routinely used for
the calibration.   
...
As illustrated on the first panel of Fig.~\ref{fig:nika_toi}, the
scaled-version of the low-frequency noise component is seen by all KIDs
at the same time, while the signal is shifted from one KID to
another. A simple co-addition of the KID timelines provides us with an
estimate of the low-frequency noise component, that we referred to as
a \emph{common mode}, while the signal is attenuated. The common mode
...
The reference method that is used for the calibration and performance
assessment, which is called the \cmoneb\ method, comprizes two
...
is mitigated by discarding on-source KID timelines from the
co-addition. Second, instead of a single common mode subtraction to
...
KID that are the most correlated with the KID under
concern. Furthermore, we have tested on simulations that this method
does not alter the flux of the source.

In Fig.~\ref{rmspws} we present the noise power spectra of
a typical datastream, either before any data reduction or using the
simple common mode method, a method using Principal Component Analysis
(PCA) and the \cmoneb\ method. We observe that after decorrelation the
...
than the common mode decorrelation at low frequencies.\\ 
...
\label{se:flux_calib}
The KID TOI in Herz are converted to Jy/beam in two steps: i) the KID
are inter-calibrated using the calibration coefficients,
\aka\ relative gains, the derivation of which
is discussed in Sect.~\ref{se:flat_field} and the
absolute scale of the flux density is set using the absolute
calibration method discussed in Sect.~\ref{se:calibration_method} and
ii) the instantaneous line-of-sight
atmospheric attenuation $\rm{exp}[{-\tau/\sin\delta_t}]$ is corrected
using the zenith opacity estimate for the scan $\tau$, which is
derived as discussed in Sect.~\ref{se:opacity} and the instantaneous observing
elevation $\delta_t$, as given using the pointing reconstruction.\\     
...
We use the pointing information to project the cleaned calibrated TOI of all the
KIDs of an array onto a map. The map is produced using an inverse
variance noise weighting of all of the data samples that fall into a map
pixel as defined using a nearest grid point scheme. The map resolution
is chosen small enough (typically $2''$ per map pixel) to alleviate the need for more refined
interpolation scheme. The noise variance of a
KID is evaluated as the standard deviation of its TOI far from the
source. Therefore, we neglect the KID-to-KID correlated noise
residuals in this procedure. We verify however that the
signal-to-noise ratio (SNR) distribution of the map is well-approximated
with a Gaussian but not normalised to unity. We correct the SNR
distribution by increasing the noise variance by a factor that goes
from 1.2 to 1.5. This constitutes an effective approach to correct
...
%paragraph.
The variance map of the sum of scans is also corrected to ensure unity
SNR distribution.
...
map is required to be lower than $12.5''$ at $1\,\rm{mm}$ and lower than $18''$ at
