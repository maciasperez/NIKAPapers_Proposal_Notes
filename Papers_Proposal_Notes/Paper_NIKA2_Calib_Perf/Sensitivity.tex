...
integration with the integration time, confirming the expected scaling
law. \nico{GIVE NEFD VALUES HERE AND/OR IN A TABLE}. The observed small
variations around the theoretical fit correspond to variations of line-of-sight
opacity during the integration. Fig.~\ref{fig:nefd_twomethods} (right panel)
shows the $\rm{NEFD}_{\tau,el}$ per scan, along with the fit that provides
$\rm{NEFD}$. Results from these analyses, per array
and for the combined A1 and A3, are presented in Table~\ref{tab:nefd_summary}.
