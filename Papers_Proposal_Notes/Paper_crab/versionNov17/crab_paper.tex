...
\textbf{Tab.~\ref{tab:peak_puls_others} reports the measurement of the polarization properties, degree and angle, performed by the POLKA, XPOL and SCUPOL experiments \cite{2014PASP..126.1027W}.
These three experiments observing at the frequency of 870$\mu$m, 3 mm, 850$\mu$m show a significant discrepancy in terms of polarization angle estimated at the position of the pulsar.
...
Tab.~\ref{tab:peak_puls_nika} show the values estimated with the \NIKA\ camera on the 2 mm maps. We present the flux densities of Stokes $I$, $Q$, and $U$, the polarization intensity flux $I_{\rm pol}$,
...
In the first case we notice that the estimated $Q\simeq\sigma_Q$, the total polarization flux is located in $U$. In this case of low S/N in $Q$ we estimate an uncertainty $\sigma_{ \rm\psi}$ which is higher than the other case. 
Generally, the values estimated with \NIKA\ are consistent within 3$\sigma$ level with POLKA and SCUPOL experiments, but they disagree the estimation of XPOL experiment which show higher values in both cases. We also notice that the total intensity flux measured varies significantly if we integrate in 2$\prime\prime$ or 18$\prime\prime$.} 
 ...
 \begin{table*}
  \centering
    ...
    \hline   
    \end{tabular}
   \caption{ Values estimated by POLKA, XPOL, SCUPOL reported in \cite{2014PASP..126.1027W} at the pulsar and peak position, respectively. }
