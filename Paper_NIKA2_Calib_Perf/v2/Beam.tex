...
of the noise decorrelation mask, which is equals to 90'', the
...
the limbs of the Moon. As EMIR is equipped with antenna feed horns,
which select the radiation illuminating the detectors, the
...
spillover and scattering efficencies are $F_{\rm{eff}}$ subtracted
from the main beam and error beams efficencies, and
the \emph{rearward} spillover and scattering efficencies equal
$1-F_{\rm{eff}}$~\citep{Kramer2013}.}

{\rev To sum up, we estimate the solid angle of the total beam as
\begin{equation}
  \Omega_{\rm{tot}}(\nu) = \int_0^{2\pi} \left( B_{\nu}(r)
  + \mathcal{A}_{\nu}^{(3)} G_{\nu}^{(3)}(r) + \mathcal{A}_{\nu}^{(4)}
  G_{\nu}^{(4)}(r) \right)  2 \pi r dr  \, + \,  \Omega_{\rm{FSL}} (\nu),
  \label{eq:omega_tot}
\end{equation}
where $B_{\nu}(r)$ is the normalised beam profile for the array $\nu$,
as discussed in Sect.~\ref{se:fullbeam}, $\mathcal{A}_{\nu}^{(3)}$ and
$\mathcal{A}_{\nu}^{(4)}$ are the amplitudes of third and fourth
Gaussian error beams $G_{\nu}^{(3)}$ and $G_{\nu}^{(4)}$ measured with
EMIR and extrapolated for the array $\nu$ following the presciption
given in \citet{Kramer2013}, and $\Omega_{\rm{FSL}} (\nu)$ is the
contribution of the far side lobes to the total solid angle of the
array $\nu$, as derived from $F_{\rm{eff}}$ measurements.}   

{\rev In addition, for the stability tests, we also compute the
...
We evaluate {\rev $\Omega_{90}$} from the measured beam profiles
...
increased to be at least equal to {\tt BE2} rms errors beforehand.
%By contrast, error
