...
conditions. We evaluate the \emph{Noise Equivalent Flux
Density}, NEFD hereafter, that is the flux density
$1\,\sigma$ error in one second of effective
on-source telescope integration time.
\nico{commented out the rest of the sentence that is not strictly part of the
  definition + by definition, the NEFD is estimated on final calibrated data.}
% after the baseline calibration
% has been performed, as described in
% Sect.~\ref{se:baseline_calibration}.
\nico{The estimation of fluxes and associated errors are presented in
  sect.~\ref{se:photometric_system} and \ref{se:map_projection}. In
  sect.~\ref{se:integration_time} we detail how the integration time is
  estimated in practice. Section~\ref{se:nefd_method} presents several methods
  of combining these parameters into NEFD and estimates. Results, together with
  robustness tests are reported in sect.~\ref{se:nefd_results}.}
