...
\abstract{The detection of the primordial Cosmic Microwave Background (CMB) polarization B-modes constitutes
  ...
  of few arcminutes corresponding to the typical beam-width of current CMB
  ...
  value of $ -88.1^{\circ} \pm 0.3$ in Galactic coordinates.  We also present the first estimation of the Crab
...
Nevertheless, as described in \cite{adam2013, catalano2014}, by integrating the Uranus flux up to 100 arcsec, we observe that the total solid angle covered by the beam is larger than the Gaussian best-fit of the main beam by a factor 1.32. As consequence we account for this factor in the estimation of the fluxes.
...
\begin{figure}[h!]
  \centering
     ...
     \caption{Cumulative flux of the Crab nebula (top) obtained at 150 GHz over 4$^{\prime}$ from the center. The flux has been corrected by a zero level in the map, which corresponds to the mean of the signal calculated in an annular ring as indicated by the white circles on the map (bottom) and by the blue dotted lines on the top. The green dotted line represents the uncertainties measured at large radii.}
...
defined regions: 5$^\prime$, 7$^\prime$, at the pulsar position and Stokes $I$ peak position.

Tab.~\ref{tab:crab_results} reports the total flux $I$, polarized intensity
$I_{pol}$, polarization degree $p$ and polarization angle $\psi$ calculated in 5$^\prime$, 7$^\prime$ FWHM from the center of the maps. 
The polarization angles are here presented in Galactic coordinates to ease the comparison with the \Planck\ \citep{2015arXiv150702058P} and \WMAP\ CMB experiments \citep{2011ApJS..192...19W}.
...
However the values obtained in Equatorial coordinates are also given within parenthesis.
...
\citep{scubapol} and \NIKA\ (this paper).  
...
Notice that, as discussed before, the XPOL/30m intensity flux is low with respect to expectations from both power law models. 
