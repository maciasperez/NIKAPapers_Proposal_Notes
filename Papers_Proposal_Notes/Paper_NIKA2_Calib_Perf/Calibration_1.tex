...
describes the absolute calibration method, section~\ref{se:flat_field} presents
the inter-calibration and the flat fields. While gathering several observations
of the calibrator in order to increase statistics and improve the calibration
overall, we have evidenced a daily variation of the absolute calibration
coefficients related to weather temperature induced variation of the beam
size. If left uncorrected, it leads to a sizable increase of the
calibration uncertainties while not being strictly related to the instrument. We
address this effect Sect.~\ref{se:beam_variation}, before summarizing the
overall calibration process in Sect.~\ref{se:baseline_calibration}.
...
\subsection{Beam monitoring and scan selection}
\label{se:beam_variation}

We evidenced variations of the flux density estimates that correlate to the
...
comes from the combinaton of two different effects.
...
telescope. {\lp To mitigate this effect, the telescope is equipped with an
  ...
  challenged when the Sun partially illuminates the telescope, like around
  sunrise and sunset.} {\lp This is a known effect that also impacts
...
been described in~\citet{Altenhoff1987}. Based on experience with
  ...
  is then observed to change by few arcseconds {\lp resulting in an average
    enlargement of the beam pattern.} This effect has been confirmed by
  measuring several arcsec displacements of the source when projected using
  different subsets of subscans of a single observation.
...
To illustrate this, Fig.~\ref{fig:beam_monitoring_otf} shows the measures of the
FWHM (cf.~eq.~\ref{eq:fwhm_geom}) for the three reference campaigns as a
function of UT for bright sources (planets or quasars of more than $1\,\rm{Jy}$ at both wavelengths). For
Uranus (or any other planet), the FWHM estimates are corrected for the beam broadening due to the
finite extension of the apparent disc of the planet, as in
Sect.~\ref{se:mainbeam}. The FWHMs go from a plateau at a median value of
$11.3''$ at $1\,\rm{mm}$ and $17.5''$ at $2\,\rm{mm}$ during the night, to a
smooth rise that reaches a maximum of about $14''$ at $1\,\rm{mm}$ and $18.5''$
at $2\,\rm{mm}$ around 16:00 UT hours. The beam broadening becomes sizable
around 15:00 UT and returns to the plateau only around 22:00 UT. This leads to
discard scans belonging to the cross-hatched intervales for the baseline scan
selection (Sect.~\ref{se:data_selection} and \ref{se:baseline_calibration}).
