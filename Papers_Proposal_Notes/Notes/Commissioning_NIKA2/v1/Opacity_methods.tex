...
Time-stamped zenith opacities at $225$~GHz $\tau_{225}$ are derived from the
...
$R^2$ is the \new{coefficient of multiple determination of the $\tau_{225}$
estimates, which quantifies the goodness of fit of each model.}
%\new{the variance of the ratio} between the two flavours of
%$\hat{\tau}_{225}$ estimates.
Redundant samples and outliers are removed.

For the NIKA2 analysis we use the linear filtered  $\tau_{225}$ data.
The time-stamped $\tau_{225}$ estimates, which are sampled about
...
produce a smooth version of time-stamped $\tau_{225}$ data by
...
masses. \samu{This method has the advantages of not relying on
...
\samu{We note that the opacity is expected to slightly depend
on the atmospheric temperature. For example, in poor weather conditions
(6 mm of water vapor contents), the zenith opacity in the $1~\rm{mm}$
observing bands can vary by about 10\% for temperature variations of
10~K. We neglect the atmospheric temperature variations. However, the
effect of the temperature variations on the final
calibration of NIKA2 response to the sky load is mitigating by using
several dedicated observation scans regularly distributed all along an
observation campaign.}

\new{We identified the main cause of variations in time of the $c_0^k$
and $c_1^k$ coefficients as the ageing of the arrays between two
cooldown cycles\footnote{The thin (18--25nm) Aluminium films from
which are made the KIDs are known to evolve over time, the main causes
being oxidation and the migration of Silicon from the substrate into
the films themselves. The main consequence of this ageing process is
that the effective pure aluminium film thickness decreases slowly with time, and the
properties of the KIDs change slightly (the kinetic inductance
increases, which causes a shift of the resonance frequencies to lower
values}. However this slow ageing effect is frozen within a cooldown cycle, and
the coefficients $c_0^k$ and $c_1^k$ are thus expected to be constant
for the duration of a run. Actually, we observed no evidence for time variations
of $c_0^k$ and $c_1^k$ during an observation campaign.}  
%The coefficients $c_0^k$ and $c_1^k$ are constant in
%time within a cooldown cycle (because the coupling of NIKA2
%to the telescope does not change),
...
\new{Once the relation between total power and opacity is calibrated, that
is the ($c_0$, $c_1$) parameters are estimated for each KID, the
opacity for each array $A_i$, $i=\{1, 2, 3\}$ can be retrieved for each observation scan by inverting
Eq.~\ref{eq:skydip} as:
\begin{equation}
...
$i$. Hence we derive an opacity integrated in the NIKA2 bandpasses and in the same line-of-sight of the
...
in Hz. \samu{This fit quality indicator is thus also sensitive to the noise
...
KID $k$. \samu{This cross-calibration allows for assembling the
...
fitting skydips, \samu{which mostly correspond to deviation from the
atmosphere plane-parallel model in high opacity skydips (as seen in
Fig.~\ref{fig:skydip-stability}),} are excluded. When these conditions are met, the
skydip selection induced uncertainties on the zenith opacities are
below $10\%$ at 1mm and below $15\%$ at 2mm. 
...
the NIKA2 skydip-based method discussed in Sect.~\ref{se:skydip-method}, and
that are referred to as 'skydip' zenith opacities $\tau_{\rm{skydip}}$
hereafter.
...
and the \samu{median-filtered time-stamped IRAM $225$ GHz taumeter 
$\tau_{225}$ zenith opacities $\tau_{225}$, which
...
\begin{figure}[ht!]
  \begin{center}
   ...
   \caption[NIKA2 skydip-based vs taumeter opacities]{ NIKA2
    skydip-based opacities $\tau_{\rm{skydip}}$ vs \samu{median-filtered
    ...
    NIKA2 frequency bands are shown in black.} 
\label{fig:skydip-to-taumeter-correl}
\end{center}
\end{figure}

We check the robustness of the 'skydip' $\tau$ against the
observing elevation. 
...
\begin{figure}[ht!]
  \begin{center}
    ...
    MWC349 scans. \samu{See discussion in Sect.~\ref{se:skydip-opacity_tests}.}} 
...
\new{For the time being, we do not know
the physical origin for the discrepancy of the empirical factor
$a_\nu^{\rm{skydip}}$ from the expected unity value.}
