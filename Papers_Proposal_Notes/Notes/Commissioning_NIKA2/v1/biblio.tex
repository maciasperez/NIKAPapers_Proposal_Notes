
\begin{thebibliography}{99}
  
\bibitem{Adam18} R.~Adam {\it et al.},
  %{\it The NIKA2 large field-of-view millimeter continuum camera for the 30-m IRAM telescope},
  Astron.\ Astrophys.\, {\bf 609} A115, 2018
  
\bibitem{Calvo16}
M. Calvo {\it et al.},  
Journal of Low Temperature Physics, {\bf 184}, 816-823, 2016 
%arXiv:1601.02774 


\bibitem{remithese} R.~Adam PhD Thesis at
  Universit\'e Grenoble Alpes, 2016\\
\url{https://tel.archives-ouvertes.fr/tel-01303736}

%NIKA
\bibitem{Monfardini11}    A. Monfardini {\it et al.}, The Astrophysical Journal Supplement 194 (2011)  24 
\bibitem{Bourrion12}    O. Bourrion {\it et al.}, JINST 11 (2016) P11001 
\bibitem{Calvo13}    M. Calvo {\it et al.}, Astron.\ Astrophys.\  {\bf 551} (2013) L12
\bibitem{Monfardini13} A. Monfardini {\it et al.}, JLTP 176 (2014) 787
    
 
\bibitem{Catalano:2014nml}
  A.~Catalano {\it et al.},
  %``Performance and calibration of the NIKA camera at the IRAM 30 m telescope,''
  Astron.\ Astrophys.\  {\bf 569} (2014) A9.
%  doi:10.1051/0004-6361/201423557
  %%CITATION = doi:10.1051/0004-6361/201423557;%%
  %5 citations counted in INSPIRE as of 16 Feb 2016
  
% NIKA2 SZ
\bibitem{Ruppin2018}
     F.~Ruppin {\it et al.}
     %{\it First Sunyaev-Zel dovich mapping with NIKA2: implication of cluster substructures on the pressure profile and mass estimate}
     Astron.\ Astrophys.\  {\bf 615} (2018) A112 

     
\bibitem{ATM}
Petit J., Bardeau S., Reynier E., ``Comparison of ATM versions: Impact
on the calibration of IRAM instruments'', IRAM memo 2009-5,
\url{http://www.iram-institute.org/EN/content-page-161-7-66-161-0-0.html}

\bibitem{hls_combes} Combes, F., Rex, M., Rawle, T. D., et al, 2012, A\&A,
  Vol.~538, L4, 2012

\bibitem{Pardo2002}
  Pardo, J.~R., Cernicharo, J. $\&$ Serabyn, E., IEEE Transactions on
  Antennas and Propagation, {\bf 49}, p.~1683, 2002
  
%@ARTICLE{2013MNRAS.433.3062B,
%   author = {{Bendo}, G.~J. and {Griffin}, M.~J. and {Bock}, J.~J. and {Conversi}, L. and 
%	{Dowell}, C.~D. and {Lim}, T. and {Lu}, N. and {North}, C.~E. and 
%	{Papageorgiou}, A. and {Pearson}, C.~P. and {Pohlen}, M. and 
%	{Polehampton}, E.~T. and {Schulz}, B. and {Shupe}, D.~L. and 
%	{Sibthorpe}, B. and {Spencer}, L.~D. and {Swinyard}, B.~M. and 
%	{Valtchanov}, I. and {Xu}, C.~K.},
%    title = "{Flux calibration of the Herschel$^{★}$-SPIRE photometer}",
%  journal = {\mnras},
%archivePrefix = "arXiv",
%   eprint = {1306.1217},
% primaryClass = "astro-ph.IM",
% keywords = {instrumentation: photometers},
%     year = 2013,
%    month = aug,
%   volume = 433,
%    pages = {3062-3078},
%      doi = {10.1093/mnras/stt948},
%   adsurl = {http://cdsads.u-strasbg.fr/abs/2013MNRAS.433.3062B},
%  adsnote = {Provided by the SAO/NASA Astrophysics Data System}
%}
\bibitem{bendo2013}
  G.~J.Bendo~{\it et al.}, MNRAS, {\bf 433}, p{3062-3078}, 2013 

\bibitem{Morenothesis}
R.~Moreno PhD Thesis at
Universit\'e de Paris, 1998
  
%Improvement of the IRAM 30m telescope beam pattern  
\bibitem{Kramer2013}
  C.~Kramer, J.~Pe\~nalver, and A.~Greve, internal IRAM 30m report,
  2013
  \url{http://www.iram.es/IRAMES/mainWiki/Iram30mEfficiencies}

%The beam pattern of the IRAM 30m telescope  
\bibitem{Greve1998}  
A.~Greve, C.~Kramer, W.~Wild, 1998a, A$\&$ASS, 133, 271

%Improvement of the IRAM 30m millimeter wavelength radio telescope from temperature measure-
%ments and ﬁnite element calculations
\bibitem{Greve2005}  
A.~Greve, M.~Bremer, J.~Pe\~nalver, P.~Raffin, D.~Morris, D.~Broguire,
IEEE Trans. Ant. Propagation, {\bf 53}, 2, 2005
  
%The beam pattern of reﬂector antennas with buckled rectangular panels
\bibitem{Greve2010}  
 A.~Greve, D.~Morris, J.~Pe\~nalver, C.~Thum, M.~Bremer, IEEE Transactions on Antennas and Propa-
 gation, {\bf 58}, 3, p. 959, 2010

 %Observations of anomalous refraction at radio wavelengths
\bibitem{Altenhoff1987}
  W.~J.~Altenhoff,  J.~W.~M.~Baars,  J.~E.~Wink, D.~Downes, 
  Astronomy and Astrophysics, {\bf 184}, no. 1-2, p. 381-385, 1987


%@ARTICLE{1991ApJ...379..271B,
%   author = {{Bieging}, J.~H. and {Wilner}, D. and {Thronson}, Jr., H.~A.
%	},
%    title = "{The molecular envelope of NGC 7027}",
%  journal = {\apj},
% keywords = {Carbon Monoxide, Emission Spectra, Line Spectra, Molecular Gases, Planetary Nebulae, Stellar Envelopes, Angular Resolution, Astronomical Maps, Ionized Gases, Radio Astronomy, Very Long Base Interferometry},
%     year = 1991,
%    month = sep,
%   volume = 379,
%    pages = {271-279},
%      doi = {10.1086/170501},
%   adsurl = {http://cdsads.u-strasbg.fr/abs/1991ApJ...379..271B},
%  adsnote = {Provided by the SAO/NASA Astrophysics Data System}
%}
\bibitem{Bieging1991}
  J.~H.~Bieging, D.~Wilner and  H.~A.~Thronson, ApJ, {\bf 379},
  pp.271-279, 1991

%Millimetre and submillimetre continuum observations of planetary nebulae
\bibitem{Hoare1992}
  M.~G.~Hoare, P.~F.~Roche~Robin and E.~S.~Clegg 
 MNRAS, {\bf 258}, no. 2, p. 257-269, 1992 

%The Evolution of NGC 7027 at Radio Frequencies: A New Determination of the Distance and Core Mass
\bibitem{Zijlstra2008}
  A.~A.~Zijlstra,  P.~A.~M.~van~Hoof and R.~A.~Perley,
  The Astrophysical Journal, {\bf 681}, Issue 2, pp. 1296-1309, 2008
  
%Radio source calibration for the Very Small Array and other cosmic microwave background instruments at around 30 GHz  
\bibitem{Hafez2008}
  Y.~A.~Hafez {\it et al.}, 
  MNRAS, {\bf 388}, Issue 4, pp. 1775-1786, 2008

%SCUBA-2: on-sky calibration using submillimetre standard sources  
\bibitem{Dempsey}
  J.~T.~Dempsey {\it et al.},  
  MNRAS, {\bf 430}, Issue 4, p.2534-2544, 2013

%Detection of radio continuum emission from circumstellar dust around CRL 2688 and IRC +10216  
\bibitem{Knapp}
  G.~R.~Knapp {\it et al.},  
The Astrophysical Journal, {\bf 429}, no. 1, pt. 2, p. L33-L36, 1994

%maging MWC 349 from 7 Millimeters to 90 Centimeters
\bibitem{Tafoya}
  D.~Tafoya, Y.~G\'omez and  L.~F.~Rodr\'iguez, 	
The Astrophysical Journal, {\bf 610}, Issue 2, pp. 827-834, 2004

\bibitem{beloche}
\url{http://www.lesia.obspm.fr/perso/emmanuel-lellouch/mars/index.php}

\bibitem{gith-Haussel-Note}
\url{https://github.com/haussel/photometry/blob/master/notebooks/planet_fluxes.ipynb}

%LII. Planet flux densities
\bibitem{PLCK-LII}Planck intermediate results LII, Planck
Collaboration, A$\&$A, {\bf 607}, A122, 2017

\bibitem{gith-Haussel}
\url{https://github.com/haussel/photometry}.

\bibitem{NASAHorizon}
\url{https://ssd.jpl.nasa.gov/horizons.cgi}

\bibitem{ESAmodel}
\url{https://www.cosmos.esa.int/web/herschel/calibrator-models}

\bibitem{krips} M. Krips, talk at the 8th 
IRAM Millimeter Interferometry School, Grenoble (France),  October 2012, and
private communication.\\
\url{http://www.iram.fr/IRAMFR/IS/IS2012/presentations/krips-fluxcalibration.pdf}

\bibitem{masingRRL} Gordon, M. A., Astrophysical Journal, vol. 421, no. 1, p. 314-317

\bibitem{durand} Durand's PhD Thesis at
Universit\'e Grenoble Alpes, 2008

%\cite{Ritacco:2016due}
%\bibitem{Ritacco:2016due}
%  A.~Ritacco {\it et al.},
  %``Polarimetry at millimeter wavelengths with the NIKA camera: calibration and performance,''
%  Astron.\ Astrophys.\  {\bf 599} (2017) A34
%  doi:10.1051/0004-6361/201629666
%  [arXiv:1609.02042 [astro-ph.IM]].
  %%CITATION = doi:10.1051/0004-6361/201629666;%%
  %2 citations counted in INSPIRE as of 21 Jul 2017
  
%\bibitem{Bracco}
%  A.~Bracco {\it et al.},
  %``Probing changes of dust properties along a chain of solar-type prestellar and protostellar cores in Taurus with NIKA,''
%  Astron.\ Astrophys.\  {\bf 604} (2017) A52
  %%CITATION = doi:10.1051/0004-6361/201731117;%%


%\bibitem{Adam:2013ufa}
%  R.~Adam {\it et al.},
  %``First observation of the thermal Sunyaev-Zel\u2019dovich effect with kinetic inductance detectors,''
%  Astron.\ Astrophys.\  {\bf 569} (2014) A66
%  doi:10.1051/0004-6361/201322902
%  [arXiv:1310.6237 [astro-ph.CO]].
  %%CITATION = doi:10.1051/0004-6361/201322902;%%
  %8 citations counted in INSPIRE as of 16 Feb 2016
  

%\bibitem{Adam:2014wxa}
%  R.~Adam {\it et al.},
  %``Pressure distribution of the high-redshift cluster of galaxies CL J1226.9+3332 with NIKA,''
%  Astron.\ Astrophys.\  {\bf 576} (2015) A12
%  doi:10.1051/0004-6361/201425140
%  [arXiv:1410.2808 [astro-ph.CO]].
  %%CITATION = doi:10.1051/0004-6361/201425140;%%
  %5 citations counted in INSPIRE as of 16 févr. 2016
  


%\cite{Adam:2015bba}
%\bibitem{Adam:2015bba}
%  R.~Adam {\it et al.},
  %``High angular resolution Sunyaev-Zel\u2019dovich observations of MACS J1423.8+2404 with NIKA: Multiwavelength analysis,''
%  Astron.\ Astrophys.\  {\bf 586} (2016) A122
%  doi:10.1051/0004-6361/201527616
%  [arXiv:1510.06674 [astro-ph.CO]].
  %%CITATION = doi:10.1051/0004-6361/201527616;%%
  %12 citations counted in INSPIRE as of 21 Jul 2017
  
  
%\bibitem{Adam:2016abn}
%  R.~Adam {\it et al.},
  %``Mapping the kinetic Sunyaev-Zel\u2019dovich effect toward MACS J0717.5+3745 with NIKA,''
%  Astron.\ Astrophys.\  {\bf 598} (2017) A115
%  doi:10.1051/0004-6361/201629182
%  [arXiv:1606.07721 [astro-ph.CO]].
  %%CITATION = doi:10.1051/0004-6361/201629182;%%
  %6 citations counted in INSPIRE as of 21 Jul 2017  

%\cite{Ruppin:2016rnt}
%\bibitem{Ruppin:2016rnt}
%  F.~Ruppin {\it et al.},
  %``Non-parametric deprojection of NIKA SZ observations: Pressure distribution in the Planck-discovered cluster PSZ1\u2009G045.85+57.71,''
%  Astron.\ Astrophys.\  {\bf 597} (2017) A110
%  doi:10.1051/0004-6361/201629405
%  [arXiv:1607.07679 [astro-ph.CO]].
  %%CITATION = doi:10.1051/0004-6361/201629405;%%
  %3 citations counted in INSPIRE as of 21 Jul 2017
  
%\cite{Adam:2017mlj}
%\bibitem{Adam:2017mlj}
%  R.~Adam {\it et al.},
  %``Mapping the hot gas temperature in galaxy clusters using X-ray and Sunyaev-Zel'dovich imaging,''
%  arXiv:1706.10230 [astro-ph.CO].
  %%CITATION = ARXIV:1706.10230;%%
%\cite{Adam:2016abn}


%\cite{Romero:2017xri}
%\bibitem{Romero:2017xri}
%  C.~Romero {\it et al.},
  %``A multi-instrument non-parametric reconstruction of the electron pressure profile in the galaxy cluster CLJ1226.9+3332,''
%  arXiv:1707.06113 [astro-ph.CO].
  %%CITATION = ARXIV:1707.06113;%%
  
  
%\cite{Comis:2016bpf}
%\bibitem{Comis:2016bpf}
%  B.~Comis {\it et al.},
  %``High angular resolution SZ observations with NIKA and NIKA2,''
%  arXiv:1605.09549 [astro-ph.CO].
  %%CITATION = ARXIV:1605.09549;%%
  %3 citations counted in INSPIRE as of 26 Jul 2017
  
  
 








\end{thebibliography}
