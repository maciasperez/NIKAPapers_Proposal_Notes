...
\abstract
    {\emph{Context:} In the past decade, sensitive, resolved Sunyaev-Zel'dovich (SZ) studies of galaxy
      ...
      profiles of galaxy clusters, non-parametric reconstructions will enable a model-independent view
      ...
      galaxy cluster CLJ 1226.9+3352 as determined from Sunyaev-Zel'dovich (SZ) data
      ...
      field of view. The overlap in spatial scales probed by these three datasets is
      therefore critical in checking for consistency between instruments.
      By using multiple instruments, our analysis of CLJ 1226.9+3352 
      covers a large radial range, from the central regions to the cluster outskirts:
      $0.05 R_{500} < r < 2 R_{500}$. This is a wider range of spatial scales
...
recover signal beyond their field of view (beyond $\sim 45$\asecs for MUSTANG and ALMA, and $\sim 120$\asecs for NIKA). As massive
...
In \citet{romero2015a}, the differences in fitted pressure profiles, especially in additional constraint of the inner pressure slope,
with the addition of MUSTANG data were noted. In the case of
\citet{romero2016,adam2015,adam2016a}, the pairs of instruments used did not have an overlap in recovered spatial scales, thus
...
However, as new SZ instruments like NIKA2
\citep[][]{monfardini2014} and MUSTANG2 \citep[][]{dicker2014a} with the ability to recover a larger range of scales come online,
there will be overlap, which for clusters observed with multiple instruments, studies of the kinetic SZ effect, or relativistic
...
leverage of the spectral distortion, where the kSZ and tSZ effects differ in their dependence on frequency.
...
analytical pressure profile based on a polytropic equation of state \citep{bulbul2010} have been explored. Of the parameterizations
of the ICM pressure profile, the generalized Navaro-Frenk-and-White \citep[gNFW][]{nagai2007} profile has garnered the most traction,
with a fairly canonical set of parameters coming from \citet{arnaud2010} (Hereafter, A10).
...
non parametrically can reveal deviations from a smooth pressure profile, which may correspond to departures from self-simlarity
...
non-parametric X-ray products, especially electron density, have \citep[e.g][]{basu2010,ruppin2017} and will provide insights
into the thermodynamic state of the ICM in clusters and likely be fundamental for improving cosmological constraints via
scaling relations.
...
scaling relations \citep[e.g.][]{kravtsov2012}
...
At a high redshift of $z=0.89$, CLJ1226.9+3352, hereafter CLJ 1227, is a massive cluster which was first discovered in the
...
From X-ray data, \citet{mantz2010} determined a scale radius $R_{500} = 1000$ kpc, which corresponds to
$M_{500} = 12.0 \times 10^{14} M_{\odot}$. In the following, we summarize how the data, which are used in this study,
were produced in previous studies.
...
\begin{table*}[]
  \caption{\footnotesize{Overview of the noise levels achieved in observations of CLJ 1227
      for each of the instruments used in this analysis.}}
\begin{center}
%\resizebox{\textwidth}{!} {
...
a few SZ effect instruments with sub-arcminute resolution.
However, MUSTANG's instantaneous field of view (FOV) limits its ability to recover scales larger than $1$\amin.
...
of 42\asec. The conversion factor from Jy/beam to Compton parameter used is $\mhyphen 2.50$, including relativistic
...
data of CLJ 1227 as presented in \citet{romero2016}.
...
The MUSTANG data map, with the point source subtracted (see Section~\ref{sec:preprocessing})
...
%cluster observation.
For this analysis, we refine the transfer function found in  \citet{romero2016} by filtering a cluster model
...
The resultant transfer function is then merged with the prior transfer function \citep[on white noise][]{romero2016}.
...
\citet{romero2016} for CLJ1227, with the use of the new transfer function in place of the standard MUSTANG
filtering procedure. We find good agreement with the previous results: for cluster models which were previously
fit with $0.5 < C_{500} < 3.3$,
%the pressure profiles are well reproduced with fitted amplitudes that are within 10\% of those
f%ound in \citet{romero2016}. Furthermore,
the best fit profile shape parameters ($C_{500}$, $P_0$, and $\gamma$) are
within $\sim10$\% agreement of the values reported in \citet{romero2016}.
...
%at \href{https://safe.nrao.edu/wiki/bin/view/GB/Pennarray/MUSTANG_CLASH}.
...
We use in this paper NIKA camera data of the cluster CLJ 1227, which were obtained at the IRAM 30 m telescope (Pico Veleta)
in February 2014, processed using the NIKA processing pipeline described in \citet{adam2014}, and  presented in \citet{adam2015}.
...
The transfer function, used in this analysis, of the processing procedure was computed using signal plus noise simulations as
...
2003 until 2012. The clusters were observed with a Lissajous pattern that results in a tapered
...
Our non-parametric pressure profile reconstruction assumes spherical symmetry and power law interpolation between bins.
Because we employ analytic integrals, we can integrate to zero and to infinity, with some clear restrictions on the power
...
in our analysis.
We perform our non-parametric fits on MUSTANG, NIKA, and Bolocam data independently, where we remove point sources from
previous determined.
...
evident in the MUSTANG data as reprocessed in \citet{romero2016}. A short VLA filler observation (VLA-12A-340, D-array, at 7 GHz)
...
is seen\citep{romero2016}. At 500 $\mu$m, \emph{Herschel}-SPIRE has a point source sensitivity of $\sim 8$ mJy,
...
We also wish to account for any mean level before fitting our cluster model. The mean level in the MUSTANG map is calculated
as the mean within the inner arcminute MUSTANG noise map. We subtract the mean level from the MUSTANG map before fitting a
cluster model. Within NIKA data, a mean level is calculated within the timestreams for data falling outside
...
the pressure distribution is normalized. Our fitting algorithm is applied
...
We note that for a non-rotating, spherical object under hydrostatic equilibrium (HSE), the power law should be
limited to $\alpha >4$ in order to avoid having infinite mass.
...
\end{equation}
wgere $G$ is the Newtonian constant of gravity and $\rho$ is the (total) matter density \citep{landau1959}.
...
that $\alpha \ge 4$ is required for a finite mass of a galaxy cluster, under the assumptions stated above.
...
Given the restrictions of ellpsoidal symmetry and a power law dependence of the integrated quantity (pressure) on the
...
In previous works, NIKA and MUSTANG noise have been taken as uncorrelated \citep[e.g.][]{romero2015a,romero2016,adam2015}.
...
corresponding filtered map of a previously determined \citep{romero2016} gNFW profile.
...
We find quantify the performance in terms of the logarithmic likelihoods ($\ln (\mathcal{L}) = -\chi^2 / 2$)
...
initial guesses), we find that the outermost bins in MUSTANG and Bolocam tend to be biased high, where in Bolocam,
the second most outer bin is also affected. We find that in the production of the models, this appears to arise
...
we find it best to retain the bins in the map fitting procedure and to trim them in subsequent analyses.
...
exclusions, are tabulated in shown in Figure~\ref{fig:nppps} and in Table~\ref{tbl:nppp_res}.
...
\begin{figure*}[h]
  ...
  \caption{Non-parametric bin correlation matrices. Left: NIKA. Middle: MUSTANG. Right: Bolocam. The coloring is
...
We wish to compare our non-parametric fits to each other and to previous results. Given the prevalence of
...
the value found in \citet{arnaud2010}. We further include nuisance parameters of calibration offsets for each dataset.
...
%    Real_Joint_gNFW_Real_11011111_2500S_500B_100W_pressure.eps
  \caption{The gNFW (parametric) pressure profile as simultaneously fit to the non-parametric pressure profiles.
    ...
    The non-parametric symbol-to-instrument association remains the same as in Figure{fig:nppps}.
    The MUSTANG and NIKA points that fall below the gNFW pressure profile (close to 0.1 $R_{500}$) are of
...
moreover, under hydrostratic equilibrium $\beta \ge 4$ would indicate an unbounded mass at arbitrarily
...
Our non-parametric fits are well reproduced with varying input parameters (Section~\ref{sec:robustness}).
...
\citet{korngut2011}, and performing a fit on virtual data, we find that a 0.5 mJy source (at 90 GHz)
...
CLJ 1227 is disturbed \citep{rumsey2016}. In particular, our non-parametric fits give an indication that the departure
...
In doing so, we cover a radial range $0.05 R_{500} < r < 2 R_{500}$, continuously recovering spatial scales in this range,
...
generality. Our quantity to be integrated along the line of sight is denoted as $\epsilon$, and has the
following behavior:
\begin{equation}
...
ellipsoidal radius, $r_e = (\frac{x^2}{a^2}+\frac{y^2}{b^2}+\frac{z^2}{c^2})^{1/2}$.
...
%\begin{align}
%  \text{Case 1} & \epsilon(r_e) = \epsilon_i (r_e^2)^{-P} &: r_e^2 \leq 1 \\
%                & \epsilon(r_e) = 0                      &: r_e^2 > 1 \\
%  \text{Case 2} & \epsilon(r_e) = 0                      &: r_e^2 < 1 \\
%                & \epsilon(r_e) = \epsilon_i (r_e^2)^{-P} &: 1 \leq r_e^2 \leq R_i^2 \\
%                & \epsilon(r_e) = 0                      &: r_e^2 > R^2 \\
%  \text{Case 3} & \epsilon(r_e) = 0                      &: r_e^2 < R^2 \\
%                & \epsilon(r_e) = \epsilon_i (r_e^2)^{-P} &: R_i^2 \leq r_e^2, 
%\end{align}
...
zero). The incomplete beta function, having $a = P -0.5$ and $b = 0.5$ suffers from undefined values
...
\begin{equation}
  ...
  \label{eqn:recibeta}
\end{equation}
allows us to extend the function into the negative domain (for $a$, which we take as $P-0.5$). 
...
The \textbf{only} case where this analytic integration fails is for $P < 0.5$ \textbf{when} integrating out to infinity,
