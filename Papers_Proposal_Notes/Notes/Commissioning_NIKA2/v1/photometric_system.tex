...
\item $B_{k} (\nu, \theta, \phi)$, the fraction of source signal
  illuminating the KID, that is the beam function. It has been
  parametrized as a function of the reference frequency $\nu_0$, 
  %[LP] effective frequency as defined in Eq.~\ref{eq:nueff0} of Sect.~\ref{se:bandpasses},
considering that its frequency dependency is only due to the
diffraction \samu{by the aperture}, hence a variation of the angular scale as $1/\nu$ from a reference frequency $\nu_0$.  
\end{itemize}
...
Because \new{the gains $G_{k}$ need to be estimated and} both $A_{p}$ and $ \Omega_{p} $ are not known with good
...
S_{s}(\nu_{0}) = \hat{S}(\nu_{0})  C_{s}(\nu_{0}, \alpha_s),
\end{equation}
\new{which depends on the reference frequency $\nu_{0}$ and the source
