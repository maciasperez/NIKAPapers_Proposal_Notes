...
pointing data are synchronized by the NIKA2 acquisition system using a
clock that gives the absolute astrononical time (a.k.a. the telescope
...
developped for the data analysis during the commissioning campaigns and
...
which impact only one sample due to the KID fast time
...
\caption[Example of Time-Ordered-Information]{Example of KID
  time-ordered information. \emph{Top:} Example of 40 KID raw TOIs during an observation
  of Uranus. The low frequency correlated component (atmospheric and electronic
  noises) is clearly seen. \emph{Bottom:} One of these TOIs (in blue) and the scaled
  \cm\ that is subtracted from it (in red).}
\label{fig:nika_toi}
\end{center}
\end{figure}
...
low-frequency noise component, which has been rescaled between KIDs,
is seen by all KIDs at the same time, while the astrophysical signal
is shifted from one KID to another. A simple average of the rescaled
KID TOIs provides us with an
...
\caption[Noise power spectra]{
  ...
  (labeled CM, green), the PCA (red) and the \cmoneb\ (cyan) methods.   
  \label{rmspws}}
\end{figure}
...
does not modify the flux of the source.

In Fig.~\ref{rmspws} we present the noise power spectra of
a typical KID TOI both before any data reduction and using three noise
decorrelation methods, which are the simple common mode method used in
Fig.~\ref{fig:nika_toi}, a method based on Principal Component Analysis
...
than the common mode decorrelation at low frequencies. The latter is
preferred over the former since it has been demonstrated to preserve
...
\label{se:flux_calib}
The KID TOI in units of Herz (frequency shifts) are converted to
Jy/beam in two steps: i) the KID data
...
atmospheric attenuation $\rm{exp}[{-\tau/\sin\elev(t)}]$ is corrected
using the zenith opacity estimate, $\tau$, for the scan, which is
derived as discussed in Sect.~\ref{se:opacity}, and the instantaneous observing
elevation $\elev(t)$, as obtained using the pointing reconstruction.\\     
...
onto a map. The map is produced using an inverse
...
the need for more refined interpolation scheme. The noise variance is
evaluated by the standard deviation of the KID TOI far from the
source. %We neglect the KID-to-KID correlated noise residuals in this
%procedure.
We observe that the
signal-to-noise ratio (SNR) distribution of the map is well-approximated
with a Gaussian but it is not normalised to unity. We correct the SNR
distribution by increasing the noise variance by a factor that goes
from 1.2 to 1.5. This constitutes an effective approach to account for
...
observation time of the day. We select the scans with
%
\begin{itemize}
...
\item[iii)] observation time from 22:00 to 9:00 UT and from 10:00 to
  15:00, that excludes the sunrise period and the late afternoon.
\end{itemize}
%
...
provided that the telescope is not heated due to observations too
close to the Sun.
