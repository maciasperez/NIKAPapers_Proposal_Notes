...
The tSZ+X-ray imaging versus X-ray only {\bf cross-correlation} is stable to within 10\%, depending on the choice of the $\ell_{\rm eff}$ model and the kSZ correction used. {\bf Changing the NIKA map zero level within its uncertainty also leads to a change of about 10\% in the absolute level of the temperature.} The {\bf cross-correlation also directly depends} on the absolute calibration of the tSZ map, which is expected to be accurate within 7\%. Model M3b provides a lower limit on $\ell_{\rm eff}$, and therefore an upper limit on {\bf the $\bar{T}_{\rm gmw}$ map. The changes in the cross-correlation as a function of the considered systematic effects are illustrated in Appendix \ref{sec:appendix}.}
...
\caption{\footnotesize{Impact of the gas density geometry model on the cross-correlation of the temperature maps. The models, $\ell_{\rm eff}$, are M1 (top left), M2 (top right), M3a (bottom left) and M3b (bottom right).}}
\label{fig:show_leff_effect}
\end{figure}

\section{Impact of the NIKA map zero level on the cross-correlation}
Figure \ref{fig:show_zl_effect} shows the impact of the NIKA map zero level on the cross-correlation between the tSZ+X-ray temperatures and the X-ray spectroscopic ones. The NIKA map zero level is estimated to $0.8 \pm 0.1$ mJy/beam (see Section \ref{sec:Thermal_Sunyaev_Zeldovich_pressure_mapping}). Here, we provide the correlation for upper (1.0 mJy/beam) and lower (0.6 mJy/beam) limits of the zero level, compared to the baseline value of 0.8 mJy/beam. The changes in the zero level result in an offset in the tSZ+X-ray temperature map, with some dependence on the temperature attributed to the gas density spatial variation.
\begin{figure}[h]
...
\caption{\footnotesize{Impact of the NIKA map zero level on the temperature cross-correlation, shown in the case of tSZ+X-ray imaging (model M1) versus XMM-\textit{Newton} spectroscopy. From top to bottom, the zero level is 0.6, 0.8 and 1.0 mJy/beam, which widely samples the systematic uncertainty in the NIKA map zero level.}}
