...
Neptune. \new{The latter is used when the former is not visible in the most
stable observing conditions.} Mars can also be used as primary
...
({\it e.g.} the latitude of \new{the 30-m telescope} as seen from the planet in the
...
143, 217 and 353 GHz bands, that would lead to \samu{a few percents accuracy}
...
\hline
%\multicolumn{6}{c}{Notes: (a) NIKA2 Run}\\
%\multicolumn{6}{c}{(b) Julian Date when the model is computed}\\
%\multicolumn{6}{c}{(c) Run duration}\\
%\multicolumn{6}{c}{(d, e) total fluxes at 260 and 150 GHz}\\
%\multicolumn{6}{c}{(f) variation of the 150 GHz flux density over the
%  duration of the run}\\
    \end{tabular}
    \caption[NIKA2 planet flux expectations]{NIKA2 planet flux expectations. Notes: (a) NIKA2 Run, (b) Julian Date when the model is computed, (c) Run duration, (d, e) total fluxes at 260 and 150~GHz, (f) variation of the 150~GHz flux density over the duration of the run}
  \end{center}
  \label{tab:fluxPred}
\end{table}
