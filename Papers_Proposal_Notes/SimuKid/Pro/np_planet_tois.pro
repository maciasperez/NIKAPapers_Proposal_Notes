...
;; Realizations
nmc = 1 ; 3 ; 1000 ;10 ;200 ;30

;; HWP power
hwp_max_ampl_jy = 0.d0 ; 70. ;50. ;100.
...
; noise
noise = randomn( seed, nsn, /double)*1d-2;*1d-1
...
;; A cause des echantillonages differents entre la simulation du signal a haute
;; frequence et la production de toi a plus basse frequence, il y a
;; necessairement une perte de flux entre le flux theorique et le flux effectif
;; de la toi produite. Il faut corriger cet effet numerique, qui revient a
;; supposer une calibration sur une jource a 1Jy par exemple, avant de
;; l'appliquer a tous les flux.
for ispeed=0, nspeed-1 do begin
   for imc=0, nmc-1 do begin
      planet_rf_flux_results[ispeed,imc,*] /= planet_rf_flux_results[ispeed,imc,0]
      planet_cf_flux_results[ispeed,imc,*] /= planet_cf_flux_results[ispeed,imc,0]
   endfor
endfor
...
plot, flux_list, planet_rf_flux_results[0,imc,*]/flux_list, $
      ...
      ytitle='Output flux / input flux (Jy)', /nodata
oplot, xra, xra*0. + 1.d0
oplot, flux_list, planet_rf_flux_results[ispeed,imc,*]/flux_list, col=col_rf, psym=-8, syms=0.5
oplot, flux_list, planet_cf_flux_results[ispeed,imc,*]/flux_list, col=col_cf, psym=-8, syms=0.5
legendastro, ['Method 1: !7e!3 = '+strtrim(epsilon_planet_rf[ispeed,imc],2), $
              'Method 2: !7e!3 = '+strtrim(epsilon_planet_cf[ispeed,imc],2)], $
             col=[col_rf, col_cf], line=[0,0], /left, /bottom
outplot, /close, /verb
