...
\end{equation}
where $A_{\rm{c}}$ is the amplitude estimate of the fixed-width
Gaussian. $\sigma_{0}$ is derived from the
...
\begin{equation}
  ...
  R_{\rm{s}}(\theta,\phi),
\end{equation}
is calibrated in Jy/FWHM$_{0}$beam. The flux density estimate for the
...
beam $\Omega_{\rm{tot}}$. The beam solid angle estimates are
...
of the calibration coefficient of each KID. {\lp They describe the
focal plane distributions of the point spread function (PSF) in
the far field of the telescope.} The calibration coefficients are
estimated using a \bm\ scan of a calibrator, as discussed in
Sect.~\ref{se:photometric_system}. We
...
Figure~\ref{fig:beam_monitoring_otf} shows FWHM$_{\rm{geom}}$ as a
