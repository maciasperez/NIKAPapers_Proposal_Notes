...
\begin{table}[!htbp]
  ...
  \begin{tabular}{r|c|c|c}
    ...
    \small{Grid step\tablefootmark{g} [$\lambda$/D] } & 1.11     &	1.10      &	0.87  \\
    %\new{with $27\,\rm{m}$ effective diameter aperture} [in $\lambda$/D]} \\
    \small{Modeled grid step\tablefootmark{h} [$\lambda$/D]}  & 1.09     &      1.09      & 0.93    \\
    \hline
  \end{tabular}
  \tablefoot{ \\
    ...
    \tablefoottext{e}{Rotation from the design to Nasmyth coordinates}
    \tablefoottext{f}{Distance between neighbour detectors}
    \tablefoottext{g}{Distance between neighbour detectors using the reference frequencies ($260\,\rm{GHz}$ and $150\,\rm{GHz}$) and a 27\,m effective diameter telescope aperture}
    \tablefoottext{h}{Distance between neighbour detectors modeled using ZEMAX simulation}
  }
\end{table}
