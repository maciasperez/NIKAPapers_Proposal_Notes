\documentclass[report,onecolumn]{aa}
\usepackage{fixltx2e}
...
the exclusion of S/N < 3 points, the values they carry forward, and
use in summary sentences and plots, are the biased ones (using S/N > 3
only). One suspects that the S/N > 3 results were ultimately chosen
...
\paragraph{\color{blue} Answer to the first question}
...
\paragraph{\color{blue} Answer to the second question}
...
\\ \\
\paragraph{\color{blue} Answer to the third question}
\textbf{(a)  In the plot of Fig. 6 the fit appears to be discontinuos because it is due to the sum of two different fits. When we DO NOT consider the Planck data for $\nu$ \textgreater 100 GHz the parameters of the fit are: A = 980.6 +- 0.7 and $\beta$ = -0.3151+-0.0002.
...
\textbf{(b) The models represented in Fig.7 account for the data at frequecies below 100 GHz to agree the  fit for Planck data represented in the plot of the polarization SED accounts for frequencies below 100 GHz. If we take into account all data we obtain:
A = 116.5+-6.52 for the amplitude and $\beta$ = -0.45+0.01.
If now we take into account all data considering an uncertainty of 0.1 fixed for all data we obtain:
