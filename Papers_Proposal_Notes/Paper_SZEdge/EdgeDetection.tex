...
%##########################           START THE PAPER           ##########################################
%###############################################################################################
\title{Application of sub-structure detection methods to resolved Sunyaev-Zel'dovich imaging of distant clusters}
%\input{listeauthors}

\date{Received \today \ / Accepted --}
\abstract {Sub-structures in the hot gas atmosphere of galaxy clusters are related to their formation history and to astrophysical processes at play in the intracluster medium (ICM). The thermal Sunyaev-Zel'dovich (tSZ) effect is directly sensitive to the line-of-sight integrated ICM pressure, and is thus particularly adapted to study ICM sub-structures provided sufficient sensitivity and angular resolution imaging are available. Here, we apply structure enhancement filtering algorithms to resolved NIKA tSZ observations of distant clusters, to search for pressure discontinuities and secondary peaks in the ICM. The same filters are applied to synthetic tSZ images extracted from the RHAPSODY-G simulations, in order to better interpret the extracted features. We also study the noise propagation trough the filters and quantify the impact of systematic effects, such as data processing induced artifacts and point source residuals, the latter being identified as the dominant potential contaminant. Half of the six cluster images we use are not deep enough to allow us detecting significant sub-structures. In the other half, we observe features that are likely related to merging events. In the triple merger \mbox{MACS~J0717.5+3745} ($z=0.55$), two strong pressure gradient ridges are observed on the east and southeast sectors, and a large gradient arc is visible on the west of the main X-ray cluster core. We also identify two main peaks in the pressure distribution, which coincide with the main sub-clusters identified at other wavelengths. However, contamination from kinetic SZ signal limits our analysis in this case. We observe a lack of tSZ compact structure in the cool-core cluster \mbox{PSZ1~G045.85+57.71} ($z=0.61$) and a tSZ gradient ridge dominates in the southeast, potentially indicating ongoing merging activity. In the highest redshift cluster, \mbox{CL~J1226.9+3332} ($z=0.89$), we detect a $\sim 45$ arcsec long ridge pressure gradient associated with a secondary pressure peak in the west region, consistent with the merger of a smaller cluster disturbing the overall spherically symmetric main cluster. Our results show that current tSZ facilities have now reach the angular resolution and sensitivity to allow us exploring the details of pressure sub-structures in clusters, even at high redshift. This work also marks the first NIKA cluster sample data release.}
