...
ensure low calibration uncertainties.
...
provided by the resident IRAM taumeter operated at $225~\rm{GHz}$;
...
ii) {\tt skydip} consists in using NIKA2 as a total-power taumeter
...
operated at 225~GHz, which {\lp about every four minutes} performs elevation scans at fixed azimuth
to monitor the atmospheric opacity.
The IRAM science support
team provides us with time-stamped zenith opacities at $225$~GHz
$\tau_{225}$, as derived from the taumeter measurements. The
...
any ATM model nor on the bandpass measurements in laboratory.
...
degrees, spanning a large range of air mass as required.
NIKA2 opacities $\tau_\nu$, for $\nu$ corresponding
to Array 1, 2, 3 and the combination of Arrays 1 and 3, are estimated
from the $225\,\rm{GHz}$ taumeter median-filtered linear-based opacity
estimates $\tau_{225}$ as
\begin{equation}  
  \tau_\nu =  a_\nu^{225}\tau_{225} + b_\nu^{225}.          
\end{equation}
The parameters parameters $a_\nu^{225}$ and $b_\nu^{225}$ are fitted
...
\end{equation}
where $\sigma$ is the rms uncertainties of the flux density estimates. Note
...
{\tt taumeter} opacities are not exactly the line-of-sight opacities
...
NIKA2 instrument itself. The {\tt taumeter} method will thus be used
...
using the NIKA2 instrument as a total-power tau-meter. This idea,
...
absolute value of its resonance frequency $f_{reso}^k$ varies with the
...
opacity and $\elev$ the observing elevation.  By assuming a
homogeneous plane-parallel atmosphere, the
airmass $x$ is defined from the elevation as $x =
\left(\sin\elev\right)^{-1}$. {\lp The Earth sphericity can be safely
...
spanning zenith opacities from 0.15 to 0.50. The horizontal axis gives the sky
effective temperature $T_{sky} = T_{atm}[1-e^{-\tau x}]$ in Kelvin, where $\tau$ is the
skydip zenith opacity found in the fit. The vertical axis shows the
resonance frequency of the KID in MHz. The blue line is the linear
...
$c_1^k \tau x$. The procedure has two steps. First, all the {\tt
skydip} scans are analysed individually to extract $f_{reso}^k$ for each
...
fitting procedure. Uncertainties on $\tau$ are estimated by doing this
procedure
...
$\tau$ values are, as expected, consistent between different detectors of
the same array.

By comparing the results of different skydips, we have verified that the
coefficients $c_0$, $c_1$ are stable, within the fitting errors, on very
...
For each {\tt skydip} scan and for each bunch of 40 KIDs, we compute the
...
quality per {\tt skydip}. First, the standard deviation of the
measure-to-model difference is calculated over all the KIDs in a
bunch. For each {\tt skydip}, we evaluate the median rms, which is the
...
{\lp Fig.~\ref{fig:skydipselection}, in
which the complementarity between the two fit-quality criteria is clearly
seen. After selection, 15 skydips are kept for the final step of the
($c_0$, $c_1$) fit in the case of the N2R9 campaign.}
...
choice of the selection criteria. We found that the $\tau$
...
are met, the {\tt skydip}-scans selection induced uncertainties on the zenith
opacities are below $10\%$ at 1\,mm and below $15\%$ at 2\,mm.  
...
line-of-sight opacity $\tau x$, only the parameter combination
$c_1^k T_{\rm{atm}}\tau$ is constrained in
Eq.~\ref{eq:skydip}. Degeneracies between $c_1^k$ parameters, the
atmospheric temperature and $\tau$ can translate into a
scaling factor in the fitted skydip opacities
$\tau_{\rm{skydip}}$. To take this effect into account, }  
we use the flux stability estimators described in
Sect.~\ref{se:taumeter-method} to fit a correction to $\tau_{\rm{skydip}}$ as
\begin{equation}  
...
under investigation. {\lp Beside the $c_1$-$\tau$ degeneracy and the effect
...
in the NIKA2 bandpasses and in the $225\,\rm{GHz}$ band. At 1\,mm, the
...
future using the foreseen in-situ bandpass measurements with a
...
and iii) are stable for various sources and scanning strategies. 
