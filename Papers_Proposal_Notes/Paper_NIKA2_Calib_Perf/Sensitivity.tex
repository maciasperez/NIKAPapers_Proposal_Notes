...
%cumulative time spent on-source by valid detectors, the latter to the cumulative time
%spent on-source by the FOV overall. %as commonly considered when talking about the
%``instrument being on-source''.
%To be even more concrete, assume that at some point during
...
%incremented while $t_{obs}$ is.
%$t_{det}$ is relevant for the estimation of detector
%performance, $t_{obs}$ is relevant for astronomers when
...
Let us consider a map of the flux density of a point-like source at a
...
% MAY-23-2019: COMMENTED OUT (LP, NP, JFMP)  
%For cross-checks, we compare the $t_{\rm{det}}$ estimates using an
...
%valid KIDs. {\lp We find the two methods to give comparable integration
%time with differences ranging from $2\%$ to $5\%$, hence
%translating into a NEFD relative difference of 1 to 2.5\%. We use $t_{\rm{det}}$
%estimates for the NEFD derivation and account for the time-estimation
%uncertainties.}   
...
%Considering the instanteneous FOV covered by the valid KIDs, we define
%the \emph{mapping speed} as the surface that can be mapped at a given
%sensitivity in one hour of integration time
%\begin{equation}
%M_s = \eta \, \frac{\pi}{4} d_{\rm{FoV}}^2 \, \frac{1}{(NEFD^2},
%\label{eq:mapping_speed}
%\end{equation}
%where $\eta$ is the fraction of valid KIDs, as given in
%Tab.~\ref{tab:number_of_kids} in Sect.~\ref{se:fov_geometry} and
...
$\sigma_S(t) = \rm{NEFD}\,e^{\taunu \, x}/\sqrt{t_{\rm{det}}}$.
Using long-time integration observation on a source, we can study the
decrease of $\sigma_S$ with integration time, and
...
%
\begin{equation}
  \sigma_S(n) = \frac{\rm{NEFD}}{\sqrt{\sum_{i=1}^{n}t_i e^{-2\taunu^i\, x_i}}},
...
estimate of $\rm{NEFD}$. This method requires long integration on a same
...
In addition, we verify that the noise averages down as expected with
...
defined as NEFD$^{(\taunu, \elev)}$ = NEFD exp${[\taunu\, x]}$. The
...
$\exp[\taunu\,x]$. These values are then extrapolated the IRAM
