...
\abstract{The detection of the primordial CMB polarization B-modes constitutes
  ...
  Crab nebula, which is a well known polarization angle calibration source. The
  Crab nebula is the most intense source in the microwave sky with an extension
  of few arcminutes corresponding to the typical beamwidth of current CMB
  experiments. The Crab nebula exhibits a highly polarized synchrotron radiation
  at radio and millimeter wavelengths.  We report in this paper high resolution
  (18$^{\prime\prime}$ FWHM) observations of the Crab nebula in intensity and
  polarization at 150 GHz with the \NIKA\ camera.  \NIKA, operated at the IRAM
  ...
  polarization angle is consistent with being constant across frequencies with a
  value of $ -88.1 \pm 0.3$.  We also present the first estimation of the Crab
  nebula Spectral Energy Distribution in polarization.  These measurements will
  be of interest for current and next generation of CMB experiments operating at
  ...
  et al.}  \keywords{Techniques: polarization -- KIDs -- individual: NIKA }
\maketitle
\begin{figure*}[h!]
  \centering
     	  { \includegraphics[width=0.32\linewidth,keepaspectratio]{figures/Crab_I_map2_2mm.eps}}	
	     { \includegraphics[width=0.32\linewidth,keepaspectratio]{figures/Crab_Q_map2_2mm.eps}}
          { \includegraphics[width=0.32\linewidth,keepaspectratio]{figures/Crab_U_map2_2mm.eps}} 
           \caption{From left to right: Crab nebula Stokes $I$, $Q$ and, $U$
             maps obtained at 150 GHz with the \NIKA\ camera. Polarization
             vectors, indicating both the degree and the orientation, are
             over-plotted in black on the intensity map where the polarization
             intensity satisfies $I_{pol} > 3 \sigma_{I_{pol}}$.}
...
lensing of the $E-$modes (\cite{ade2015planck} and refs. therein) {\bf I add the
  ``refs. therein'' because this paper is not the ``discovery paper''}.  The
detection of the primordial $B-$modes could definitively probe the existence of
...
the extension of the source.  For a more detailed review on this source we refer
to \citet{2008ARA&A..46..127H}. {\bf It is also quite isolated with low
  background diffuse emission.}
...
wave plate (HWP) followed by an analyzer, {\bf both at room temperature and
  placed in front of the entrance window of the cryostat. The KID's are not
  intrinsically sensitive to polarization. The HWP rotates at XX~Hz which
  modulates the polarization signal at $4\times XX$~Hz. With a typical scanning
  speed of 25 (TBC) arcsec/s, this provides a quasi-simultaneous measure of $I$,
...
\item Subtraction of the atmospheric emission in the demodulated TOIs using
  ...
  the large angular scales we use {\bf the 260~GHz band as an atmosphere
...
\item Correction of the intensity to polarization leakage effect, which was
  ...
  point sources the effect is about 3\% peak-to-peak {\bf but can be reduced to
    less than 1\% \citep{ritacco2017} before projecting the maps.} {\bf Je
      pense qu'il ne faut pas parler ici des 0.5\% dans le cas d'une source
      diffuse. En effet, ca n'est vrai que pour des sources etendues et
      d'intensite constante sur des echelles plus grandes que le beam. Dans le
      cas de ce travail, justement on veut montrer qu'on observer la granularite
      de l'interieur du crabe, donc on ne veut pas suppose a priori que c'est
      faiblement variable. ET comme en plus notre algorithme reduit l'effet du
      leakage, y'a pas besoin d'en rajouter.}
%, while for extended sources
%  like the Crab nebula is of the order of 0.5 \% peak to peak. After correction
%  the leakage effect in extended sources is negligible.
...
condition $I_{pol}=\sqrt{Q^2+U^2}$ $\textgreater$ 5 $\sigma_{I_{pol}}$ {\bf je
  propose de noter $I_{pol}$ plutot que $P$, parce que dans les equations, il
  est difficile de faire la difference entre $\sigma_p$ et $\sigma_P$}. The
...
function of total intensity satisfying the condition $I_{pol} > 4\sigma_{I_{pol}}$.
...
good understanding of the Crab polarized emission proprieties {\bf as well as
...
5$^\prime$ region {\bf Important to note here that this ``average'' is the
  integrated (isn't it ?) value over a 5~arcmin GAUSSIAN beam. If you don't,
  some people might think you've just ``top hat'' averaged, which would be
  wrong.} and considered only pixels for which $I_{pol} > 3\sigma_{I_{pol}}$.
...
which was estimated considering all available observations but SCUPOL, is -88.1
...
%            seen by 5$^{\prime}$ & 219.0$\pm$0.1  & 14.8 $\pm$0.03 & 6.78$\pm$0.01 & -83.7$\pm$0.1$\pm$1.8(syst)    \\ 
  ...
  %    P $\textgreater$ 3 $\sigma_P$     & 84.6$\pm$0.01  & 10.51 $\pm$0.01 &12.42$\pm$0.01 & -87.15 $\pm$0.04$\pm$1.8(syst) \\
   ...
   \caption{ Total intensity flux $I$, polarized intensity flux $P$,
     ...
     in regions with high {\bf donne plutot la valeur du critere plutot que
       juste ``high'' qui est imprecis et subjectif. Et plutot que ``seen
       by''dans le tableau, mettre ``FWHM='' pour etre moins ambigu.} S/N and within
     7$^{\prime}$, 5$^{\prime}$ radius from the center of the source, and around
     ...
     1.8$^{\circ}$ is also considered.  The polarization angle estimation
     satisfies the condition P $\textgreater$ 3$\sigma_P$ to avoid noise bias.
...
to the intensity data, we find no clear hint of a break in the polarisation SED
at frequencies above 90 GHz, {\bf hence putting pressure on the hypothesis of
...
-88.1$\pm$0.3 {\bf in Galactic coordinates.}

Moreover we have characterized the intensity and polarization SED of the Crab
nebula. In intensity recent \Planck\ data show a break in the SED {\bf that
...
the physics at play. {\bf Among future polarization experiments,
  \NIKAd\ in particular (ajouter une ref a un Monfardini et al), will add a
