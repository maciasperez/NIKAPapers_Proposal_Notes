...
KID timeline whose width \new{is of 31 samples, that is equivalent to about 
5~FWHM at 65 arcsec/s given that the sampling frequency is 23.84
Hertz,} and we project one map per KID in Nasmyth
...
the geodesic convergence at the poles. \samu{Moreover, basic tests on
  the time stamping of the samples are performed and used to flag out
  time-stamping failures, so that the impacted samples are not
  projected onto the map}. We then fit a 2D elliptical Gaussian on
...
\caption[KID selection and stability of position in the FOV]{On each of these
  ...
  positions of each valid KID, as obtained from each \bm. \new{For each
  KID, positions estimated in consecutive \bms\ are linked with
  a solid line to highlight those that are not found at the same position
  for all the \bms. These KIDs, which change positions in the FoV,
  are discarded.} Units are arcseconds.}
...
differs by 7 and 2\,arcsec \samu{in Nasmyth coordinates} from the 2\,mm array center. The sampling is above
$\lambda/D$ at 1\,mm, assuming \new{a 27\,m effective diameter aperture.} Note that
...
\caption[Simulated FOV grid]{NIKA2 grid diagram simulated using
  ...
  $6.5\,\rm{arcmin}$ diameter FOV. Blue crosses indicate
  the simulated ray tracing coordinates in the local image plan (which
  transforms into Nasmyth plan by a clockwise rotation of $76.2^o$).} }
...
%The quoted measured distortions are not
%too dissimilar once the different fitting methods have been taken into
%account.
\samu{The quoted distortions between the measured and designed positions are
consistent with the expected maximum distortions from the NIKA2
optics.}
...
\caption[Focus surfaces]{Focus surface of A1, A3 and A2 arrays from left to
  ...
  Gaussian. \new{On each plot, the x and y axis are the Nasmyth offsets
  w.r.t. the center of the array in arcsec, while the color-code represents
  the relative focus estimate to the central focus, given in mm.} }
...
per array that are in good agreement with each others. \samu{Furthermore,
...
with slightly higher curvature amplitude.} %{\bf [TBA, as discussed further below]}.
...
$0.015~\rm{mm}$. \todo{Marco: to be compared to the precision of the
  setting of M2 position.} 
