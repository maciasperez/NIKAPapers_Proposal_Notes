...
manually KIDs that show a cross-talk counter part on their map. \todo{show maps
  of ``doubles'' to illustrate this}
...
estimate of each KID offset, it is possible to apply the pointing reconstruction
...
planet provides the required intercalibration to average all the timelines into
a single map. This improved timeline processing allows an improved derivation of
the flux as observed by each KID, up to an opacity correction that will be
performed later on \todo{XXXX CHECK REF TO ABS CAL SECTION XXX}(see sect.~\ref{se:abs_cal}.)
...
%----------------------------------------------------------------------------------------
\section{FOV grid distortion {\color{blue} Nico et Xavier }  }
...
We measure NIKA2 focal surfaces by means of a sequence of five 'beam-map'
