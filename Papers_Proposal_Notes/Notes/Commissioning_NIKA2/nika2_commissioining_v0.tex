...
\includegraphics[clip, angle=0, scale = 1]{Figures/nefd_mpfit_HLS091828.eps}
...
The data were decorrelated using the {\tt common\_mode\_one\_block} method: for
each KID $k$, we look for the kids that are most correlated to it and build a
``common mode'' with them that we subtract from $k$, masking out the samples
that are less then XX arcsec. The scans have been combined with standard inverse
noise weighting. The noise in each map pixel is derived from the rms of the
background corrected for the number of observations per pixel.
...
or fix the power law to -0.5 and fit only the amplitude. The displayed error
bars are indicative. They have been estimated via a bootstrap method: we
randomize the scans and derive the standard deviation of the average of $n$
scans for any $n$ between 1 and the total number of scans. This gives us an
estimate of the uncertainty on $\sigma_\phi$ for a time of integration
corresponding to $n$ scans. Strictly speaking, all the scans do not have the
exact same duration, but the difference is negligible here. The number of
possible independents sets of $n$ scans is limited though, which correlates our
error bars. This is why we take them as indicative only and why the displayed
value of the $\chi^2$ should be taken at face value. Despite these caveats, the
power law fit looks robuts and the consistency between both methods (and the
third one described in Sec.~\ref{se:nefd_air_mass}) is good.
...
{\bf Need to add figures from Juan and Pablo and their comments to detail on the results}
