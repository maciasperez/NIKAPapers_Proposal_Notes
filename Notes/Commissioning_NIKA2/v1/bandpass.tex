...
\begin{equation}
\nu_{\rm{eff}}( \delta, mm_{H_{2}O}) = \frac{ \int_{0}^{+\infty} S_{\nu}
  T_{\nu}(\delta, mm_{H_{2}O}) \nu d\nu } { \int_{0}^{+\infty} S_{\nu} T_{\nu} d\nu}
\label{eq:nueff0}
\end{equation}
...
\begin{equation}
\Delta\nu ( \delta, mm_{H_{2}O}) = \int_{0}^{+\infty} \frac{T_{\nu}( \delta, mm_{H_{2}O})}{Max(T_{\nu}( \delta, mm_{H_{2}O}))}d\nu
\end{equation}
...
\caption[Atmosphere effect on system transmission]{Variations of (a) the
  ...
  elevations referred to as $e$ in the legend. \todo{modifier le titre y en $(\nu_{eff} - \nu) \ \nu$
    et la legende e en $\delta$} }  
