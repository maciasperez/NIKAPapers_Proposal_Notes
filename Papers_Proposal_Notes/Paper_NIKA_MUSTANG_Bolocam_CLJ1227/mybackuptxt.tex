...
redshift range $ 0.25 < z < 0.89$. We compare our pressure profiles to those from the X-ray data 
presented by the ACCEPT collaboration, both under the assumption of spherical symmetry. 
The fitting procedure simultaneously
fits spherical cluster models to MUSTANG and Bolocam data. In this analysis, we adopt the 
...
are consistent with those in previous studies, but on individual clusters we find discrepancies 
with pressure profiles from ACCEPT. We find for all clusters in our sample: 
...
substructure. Three of the fourteen clusters have established substructure, while an additional
...
\begin{figure*}[!h]
  \centering
  \begin{tabular}{cccc}
   \epsfig{file=figures/Abell1835_MBX.eps,width=0.25\linewidth,clip=}   &
   \epsfig{file=figures/Abell611_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS1115_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS0429_MBX.eps,width=0.25\linewidth,clip=}    \\
   \epsfig{file=figures/MACS1206_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS0329_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/RXJ1347_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS1311_MBX.eps,width=0.25\linewidth,clip=}    \\
   \epsfig{file=figures/MACS1423_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS1149_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS0717_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/MACS0647_MBX.eps,width=0.25\linewidth,clip=}    \\
   \epsfig{file=figures/MACS0744_MBX.eps,width=0.25\linewidth,clip=}    &
   \epsfig{file=figures/CLJ1226_MBX.eps,width=0.25\linewidth,clip=}    &
     &
  \end{tabular}
  \caption{MUSTANG maps of the remaining 5 clusters in our sample. Pale contours are MUSTANG contours;
    black contours are Bolocam. Both start at $3\sigma$ decrement, with $1\sigma$ intervals. 
    Red contours are X-ray surface brightness contours at arbitrary levels. The red asterisk is the 
    ACCEPT centroid; the black asterisk is the Bolocam centroid.}
...
\tablecomments{Subscripts $_{B}$ and $_{M}$ denote Bolocam and MUSTANG properties respectively. Noise$_{B}$
  ...
  given instruments. $A_{10}$ values indicate the significance (in $\sigma$) of a spherical gNFW profile
  (see Section~\ref{sec:bulk_ICM}) using parameters from \citet{arnaud2010}, 
  with normalization, $P_0$, allowed to vary.}
\end{deluxetable*}
