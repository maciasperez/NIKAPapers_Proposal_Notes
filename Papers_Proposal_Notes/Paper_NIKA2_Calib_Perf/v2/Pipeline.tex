...
from the pointing information (Sect.~\ref{se:ptg}), that is zero if
the KID is close to the source, and unity otherwise. In the case of a
point source, the mask consists in a disk of a minimum radius of 60''
centered on the source, whereas for diffuse emission, tailored masks
driven by the source morphology are build using iterative methods, as
for example in~\citet{Ruppin2018}.}
...
sources.}
\rev{Regarding diffuse emissions, the noise decorrelation induces a
filtering effect at large angular scales that required to be corrected
...
processing transfer function using simulation, is
...
concern, and requires to construct \emph{a minima} a toy model of the
source in order to inject synthetic signal in the simulation. An
example of the data processing transfer function for
NIKA2 observation toward a galaxy cluster is given
