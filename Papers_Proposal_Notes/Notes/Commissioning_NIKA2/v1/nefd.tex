...
(dec,ra), (az,el), (el,az). \samu{Although this scan strategy was not
required for the NEFD assessment, we take advantage of this long
duration observation to also test the impact of the scan
orientation. The results of this additional study will be presented in
a compagnion published article. A similar scan strategy is used for
diffuse emission study, since it makes the filtering due to the data
reduction more isotropic to avoid the presence of spurious priviledged directions in
the map. It also make possible the use of dedicated denoising
algorithms (a.k.a. destriping codes), which requires redundancy in the
scanning strategy.}
