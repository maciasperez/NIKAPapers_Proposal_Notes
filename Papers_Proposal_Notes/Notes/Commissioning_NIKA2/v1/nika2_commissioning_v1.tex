...
\begin{abstract}
  ...
  thousands observation scans that were acquired at the February
  ...
  January 2018. This vast data set allow us to check the performance
  statibility for one year and against a large range of observing
  elevations and atmospheric conditions. The calibration and
  performance assessment methods that we have developped are also
  thoroughtly described, as well as the robustness tests that we have
  performed. 
...
\label{se:opacities}
\input{Opacity_methods}  % Xavier&Laurence
\input{Opacity_measures} % Juan&Laurence
...
\input{beam_fullb.tex}
\clearpage
% Main Beam
\input{beam_mainb.tex} % Laurence&Jean-Francois
% Beam efficiency 
\input{beam_efficiency.tex} % Laurence&Jean-Francois
% Stability of the Beam Pattern
%\input{beam_stability.tex}
