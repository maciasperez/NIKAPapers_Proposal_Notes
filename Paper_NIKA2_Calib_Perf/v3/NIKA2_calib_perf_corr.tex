...
planetary systems, Galactic sources, %\LEt{Please capitalise Galaxy and Galactic (adj.) when referring to \textit{our} local Galaxy. Please use lowercase when referring to other galaxies. Please check for this throughout for consistency. Thank you}
...
referred to as the \baseline\ %\LEt{A\&A does not allow italics or other non-Roman fonts to show emphasis (neither "textit" nor "emph" LaTeX commands). Please remove this formatting here and throughout your paper. See Sect. 1.2 of the Language Guide (https://www.aanda.org/for-authors/language-editing/1-introduction) }
...
Once the instrument is correctly focused, we are able to %\LEt{? Please remember to review tenses throughout}
...
$65''\rm{s}^{-1}$.%\LEt{Please be cautious in the use of the slash. This sign is used first and foremost to indicate a ratio and secondly to replace or. Please check for this throughout to avoid misinterpretation. For details, refer to Sect. 3.6 of Language Guide. (https://www.aanda.org/for-authors/language-editing/1-introduction) }.
...
applied in the FOV geometry obtained from the \bm\ scan, as discussed in Sect.~\ref{se:fov_geometry}. %Sect.~\ref{se:avg_kidpar}.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{r|c|c|c}
    ...
    \small{Grid step\tablefootmark{h} [arcsecond, mm] }     & 9.8, 2.00 &  9.7, 2.00  &    13.3, 2.75 \\
  ...
  position is less than $4''$ away from the expected position;}}           
    \tablefoottext{c}{{\lp Median angular offset [arcsecond] between the expected and measured sky position of the KIDs;}}
    \tablefoottext{d}{{\lp Average best-fit cross term of the polynomial model across the FOV [arcsecond];}}
...
$\rm{mm}$ and arcseconds. %\LEt{Sometimes you use arcsec, sometimes arcesconds, could we choose one for consistency?} 
...
primarily used %\LEt{?? Reminder to check tenses throughout. Is this your method? Or  a common fact...?} 
...
Namely, we used \bm\ scans of Uranus%(scan id '20170125s223' and '20170125s243')
...
are evaluated as the standard deviation of the best-fit %\LEt{Is there a difference between bes-fit and best-fitting? If not, could we go for consistency?    } 
...
{\rev The main-beam solid angle $\Omega_{\rm{mb}}$ is evaluated from the main beam %(MB)\LEt{If you'd like to abbreviate main beam, perhaps do this from the first use and only use MB thereafter?}
...
$e^{\taunu\,x}$ as given in Eq.~\ref{eq:nefd_tau_el}. %for a background-limited sensitivity model.
...
\begin{tabular}{rrrcl}
  ...
  \noalign{\smallskip}
    ...
    \hline
    \noalign{\smallskip}
    Rms pointing error    [arcsecond]               & $<3$ &  $<3$  & & Sect.~\ref{se:pointing} \\
    \hline
    \noalign{\smallskip}
  ...
  \tablefoot{ \\
    ...
    \tablefoottext{f}{Ratio between the reference FWHM beam and the
      ...
      side lobes}
    \tablefoottext{f}{Systematic calibration uncertainties due to the opacity correction using the {\tt corrected skydip} method estimated at the reference IRAM \trentemetre\ Winter observing conditions: 2-mm pwv, $60^{\circ}$ elevation} 
    \tablefoottext{h}{Effective power law of noise reduction with integration time}
    \tablefoottext{i}{NEFD at zero atmospheric opacity}
    \tablefoottext{j}{Mapping speed at zero atmospheric opacity}
...
pointing-based FWHM
%$_{\rm{geom}}$
is filtered with a running median on a 70-minute-width time window. Then, the FWHM %$_{\rm{geom}}$
