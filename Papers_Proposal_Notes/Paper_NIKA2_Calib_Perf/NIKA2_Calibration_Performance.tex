...
% bleu  0, 153, 255 
% vert  0, 179, 0
%\newcommand{\lp}[1]{\bf \color[RGB]{0, 102, 255} #1}
...
\begin{document} 
   ...
   \author{L.~Perotto \inst{\ref{LPSC}}
     ...
     \and  R.~Adam \inst{\ref{LLR}} %\inst{\ref{LPSC},\ref{OCA},\ref{CEFCA}}
...
%  
   \institute{
     ...
     \label{IPAG}
     \and
     Laboratoire Leprince-Ringuet, Univ. Paris-Saclay, \'Ecole polytechnique, F-91128 Palaiseau, France
     \label{LLR}
     \and
...
\begin{tabular}{rrrcl}
  ...
  \noalign{\smallskip}
    ...
    \hline
    \noalign{\smallskip}
    Systematic calibration uncertainty [\%]      &   5         & 5 & & Sect.~\ref{se:photometry_criteria}, Appendix~\ref{se:ref_flux_uranus_neptune} \\
    Rms calibration error [\%]                   &   5.7       &     3.0       & & Sect.~\ref{se:photometry_baseline} \\
    \hline
    \noalign{\smallskip}
    $\alpha$ noise integration in time\tablefootmark{g}\hspace{3mm}  & 0.5 & 0.5 & & Sect.~\ref{se:nefd_results} \\
    \hline
    \noalign{\smallskip}
    NEFD\tablefootmark{h}\hspace{3mm} [$\rm{mJy} \cdot \rm{s}^{1/2}$]  & $30 \pm 3$  & $9 \pm 1$ &  & Sect.~\ref{se:nefd_results}\\
    M$_{\rm{s}}$\tablefootmark{i}\hspace{3mm} [arcmin$^2 \cdot
  ...
  \tablefoot{ \\
    ...
    \tablefoottext{f}{Ratio between the reference FWHM beam power and the total beam power up to a radius of 180''} 
...
\item %\emph{Beam Properties:}
  ...
  main beam efficiency up to a radius of $180''$. We found main beam
...
\item %\emph{Calibration uncertainties:}
  ...
  flux density is above about one Jy. We find rms calibration
...
\item %\emph{Sensitivity:}
  ...
  have derived robust estimate of the NEFD using more than a
  thousand scans encompassing a large range of observing
  conditions. We found NEFD at zero atmospheric opacity of
  $30 \pm 3\,\rm{mJy}\cdot s^{1/2}$ at
  $1\, \rm{mm}$ and $9 \pm 1\,\rm{mJy}\cdot s^{1/2}$ at $2\, \rm{mm}$.
  ...
  about one hundred scans of \hls\,. The conservative NEFD
  estimate reaches the goal value at $2\,\rm{mm}${\lp, which is of
  ...
  $1\,\rm{mm}${\lp, which is of $30\,\rm{mJy}\cdot s^{1/2}$ at the IRAM
  \trentemetre\ telescope reference observing
  conditions.} Indeed, the instrumental sensitivity at $1\,\rm{mm}$
  is currently mainly limited by the non-optimal transmission of
...
\item  NIKA2 mapping capabilities are better estimated by evaluating the
 ...
 \trentemetre\ telescope resident instruments~\citep{Catalano2014, Staguhn2011_GISMO, Kreysa1999}.
...
at least for the next decade.} NIKA2 performance meet the requirements
to address some of the most exciting open questions in astrophysics
and cosmology.
