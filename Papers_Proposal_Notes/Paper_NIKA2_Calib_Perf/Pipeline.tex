...
detection. Here we summarize the main steps of the data reduction.
...
while the astrophysical signal is shifted from one KID to another.
{\lp In this figure, the KID timelines have been rescaled with respect to
the average signal.} 
...
{\lp For the calibration and performance assessment, we use a noise
...
atmospheric attenuation $\rm{exp}[{-\tau x(t)}]$ is corrected
...
$x(t)$. {\lp The latter is estimated as $1/sin(\elev_t)$ using the observing elevation
...
the need for more refined interpolation scheme. The noise variance for
each KID is evaluated by the standard deviation of the KID TOI far
from the source. %We neglect the KID-to-KID correlated noise residuals in this
%procedure.
...
correlated noise while evaluating the map variance, we resort to an
effective approach.
First, we observe that the signal-to-noise ratio (SNR) distribution of
the map is well-approximated with a Gaussian but it is not normalised
to unity. This is due to the remaining correlations between KID TOIs
before projection. Then, we increase the width of the Gaussian noise,
which actually increases the map variance by the required factor so
that the SNR distribution becomes normalized.
%Then, we normalise the SNR distribution by
%increasing the noise variance by a factor that goes from 1.2 to 1.5.
This normalization factor ranges from 1.2 to 1.5 depending on the
observing conditions. This
