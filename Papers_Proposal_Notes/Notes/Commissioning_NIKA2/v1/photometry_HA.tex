...
\item $A\Omega (\nu) $: the KID \emph{etendue}, {\it i. e.} the product of
  ...
  operation (2.75 mm at 2 mm for example). The collecting area $A$, \samu{
  that is the entrance pupil of the system, is the image of the
  aperture stop (\aka\ the cold stop) through 
  the optical system; it is situated near the primary mirror (M1). The entrance pupil is made such
  that the edges of M1 are vignetted in order to avoid strong diffraction effects due to
  the high contrast between the sky effective Rayleigh-Jeans
  temperature inside M1, which is about 30~K and
  the ground temperature behind M1 (~ 300K). 
  %So the KID never seed the M1 edges, but only the sky up to the cold
  %pupil edge. So in fact
  %AOmega = A_KID* Omega_cold_pupil_projection_on_last_lens
  %       = A_cold_pupil_projection_on_M1 * Omega_KID_projection_on_sky
  } 
...
%\begin{equation}
%\nu_{\rm{eff}}( \am, mm_{H_{2}O}) = \frac{ \int_{0}^{+\infty} I(\nu) \frac{T'(\nu)}{\left(\frac{\nu}{\nu_{0}}\right)^{2}} e^{\left(-\am
%  . \tau(\nu,  mm_{H_{2}O})\right)} \nu d\nu } { \int_{0}^{+\infty} I(\nu) \frac{T'(\nu)}{\left(\frac{\nu}{\nu_{0}}\right)^{2}} e^{\left(-\am
...
%hPa. $\nu_{\rm{eff}}$ allows us to characterise the impact of the variation of
