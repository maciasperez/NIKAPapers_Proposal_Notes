...
%Their optimal operation is achieved at a temperature of around
%150\,mK, well below the Aluminium superconducting transition.
For this reason,
...
%Fourier components that are In-phase ($I$) and in Quadrature $Q$
...
%Cooper pair breaking, and thus to
...
%the excitation signal used for the readout
...
conditions.
%for the KIDs to be tuned to the background conditions.
...
%point.
%{\lp in presence of small power loading differences.}
...
\caption[NIKA2 transmission]{Relative system response of the three KID
  ...
  Neptune
%\footnote{These models are available in the Herschel
...
responses for two polarizations of the detector~\citep{Adam2018, Shu2018_bandpass}. 
%For
...
frequency $\nu_{\rm{eff}}$ as the integral over the bandpass of the
observing frequency for Uranus in various atmospheric conditions.} In
Table~\ref{tab:frequencies}, we give $\nu_{\rm{eff}}$ and the
bandwidths computed both at zero atmospheric opacity and for the
