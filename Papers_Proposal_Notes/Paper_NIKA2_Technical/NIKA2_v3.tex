...
\begin{figure}[h]
   \centering
   \includegraphics[width=.95\linewidth]{NIKA2_cryoStages.pdf}
      \caption{(Color online) Cross-section of the NIKA2 instrument showing the different cryogenics stages. The overall lenght of the instrument is around 2.3 meters. The weight is close to 1300 kg.}
         \label{Cryostat_cryo}
\end{figure}
...
The operation of the cryostat does not require cryogenic liquids, and is fully remotely controlled. The whole cool-down procedure lasts about five days. In particular, four days are required for pre-cooling the coldest stages at around 4 K. During the last 24 hours, the dilution procedure is started, allowing further cooling down to base temperature. The system is designed for long observational runs, and has showed the needed stability of the base temperature over one to two month so far. The stability of the detectors temperature is better than 0.1 mK over the duration of a typical observational block (scan), i.e. 15 minutes. 
