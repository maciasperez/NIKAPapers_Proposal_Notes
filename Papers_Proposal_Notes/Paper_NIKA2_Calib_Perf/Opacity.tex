...
The IRAM 30-m telescope facility is equipped with a resident \taumeter
...
\begin{table}[!htbp]
  \begin{center}
    \caption[IRAM \taumeter to NIKA2 opacity model]{IRAM \taumeter to NIKA2 opacity model.}
...
using the NIKA2 instrument as a total-power \taumeter. This idea,
...
%
\begin{equation}
f_{reso}^k  = c_0^k - c_1^k T_{atm}[1-e^{-\taunu x}]
\label{eq:skydip}
\end{equation}
%
...
$T_{atm}$ is the temperature of the atmosphere, $\taunu$ the zenith
...
effective temperature $T_{sky} = T_{atm}[1-e^{-\taunu x}]$ in Kelvin, where $\taunu$ is the
...
fitting procedure. Uncertainties on $\taunu$ are estimated by doing this
procedure
...
$\taunu$ values are estimated for each skydip (as the average over the
blocks), we compute, while fixing the $\taunu$, the $c_0$ and $c_1$
...
\begin{equation}
\taunu =   \rm{Med}\left( -\frac{1}{x} \log\left( \frac{f_{reso}^k - c_0^k}{c_1^kT_{atm}} +1 \right)\right), 
\end{equation}
...
under investigation. {\lp Beside the $c_1$-to-$\taunu$ degeneracy and the effect
...
measured {\tt skydip} $\taunu$. This mild discrepancy with the
...
We further check the robustness of the {\tt skydip} $\taunu$ against the
observing elevation. 
